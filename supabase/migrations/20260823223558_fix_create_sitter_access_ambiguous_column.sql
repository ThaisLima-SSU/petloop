/*
# Fix ambiguous column reference in create_sitter_access

1. Modified Functions
   - Qualify the `token` and `expires_at` column references in the SELECT
     INTO statements with the table alias `sat.` so they are not ambiguous
     with the PL/pgSQL output variables of the same name.

2. Data Safety
   - No rows or columns changed.

3. Security
   - SECURITY DEFINER, search_path, and execute grants preserved.
*/
